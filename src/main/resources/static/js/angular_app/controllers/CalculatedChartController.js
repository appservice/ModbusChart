 ...
 */
angular.module('myApp.controllers').controller("CalcuatedChartController", function($scope, Restangular,$rootScope) {
	...
	$scope.currentPrice=$rootScope.preferences.currentPrice;
//	$scope.squaresTable = [];
	...
	Restangular.one('rest/servers', 1).get().then(function(myServer) {
		...
		//	console.log(returnedData);
...

			{
				...
				//$scope.currentValue=0;
				var sum=0;
				for(k=0;k<returnedData.values.length;k++){
					sum+=returnedData.values[k];///returnedData.energyConsumption*$rootScope.preferences.currentPrice;

					
				}
				
				console.log('energy: '+energyMock);
				console.log('sum '+sum);
				$scope.currentValue=/*returnedData.energyConsumption*/energyMock*$rootScope.preferences.currentPrice/sum;
